...
<div class="admin-biaogelist">
    <div class="listbiaoti am-cf">
      <ul class="am-icon-flag on"> 查询可登记发票</ul>
      
      <dl class="am-icon-home" style="float: right;">当前位置： 首页 > <a href="#">可登记发票查询</a></dl>
      
    </div>
	<form class="am-g" action="<%=request.getContextPath()%>/invoicecxdj.action" method="post">
		...
		<table width="90%" class="am-table-bordered am-table-radius am-table-striped am-table-hover">
			...
			<c:forEach items="${list}" var="u">
				<tr>
					<td>${u.id}</td>
					<td>${u.price}</td>	
					<td><a class="am-btn am-btn-default am-btn-xs am-text-success am-round" href="<%=request.getContextPath() %>/updatedj.action?id=${u.id}"><span class="am-icon-search" title="丢失">登记</span></a></td>
				</tr>
