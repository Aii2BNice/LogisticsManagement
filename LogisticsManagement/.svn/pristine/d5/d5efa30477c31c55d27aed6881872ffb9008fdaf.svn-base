...
<script type="text/javascript">
	$(function(){
		...
		$("#powername").blur(function(){
			$.ajax({
				type:'post',
				url:'${pageContext.request.contextPath}/verifictionpower.action',
		...
		function getJsonData(){
			var json = {"powername":$("#powername").val()}
			return json;
		}
	})
</script>
	...
	<div id="main">
		...
		<div class="admin-content" id="main-right">
			<div class="listbiaoti am-cf">
				<ul class="am-icon-flag on">增加权限</ul>
				<dl class="am-icon-home" style="float: right;">当前位置： 系统管理 ><a href="#">日志管理</a> ><a href="#">查询日志</a></dl>
			</div>
			<h2>增加权限</h2><br /><br />
			<form action="<%=request.getContextPath()%>/insertpower.action"
				method="post">
				<table class="am-table-bordered am-table-radius am-table-striped am-table-hover">
					<tr>
						<td>权限名:</td>
						<td>
							<input type="text" name="powername" id="powername"  required="required"/><span id="span"></span>
						</td>
					</tr>
					<tr>
						<td colspan="2">
