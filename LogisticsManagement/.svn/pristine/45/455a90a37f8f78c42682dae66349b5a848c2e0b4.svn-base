...
public class UserController {
	...
	public String deleteUser(HttpServletRequest req, String act) {
		...
		journalService.insert(new Journal(null, "su", null, null, null, null, "删除用户"));
		if (count > 0) {
	...
	public String updateUser(HttpServletRequest req, User user) {
		...
		try {
			...
			journalService.insert(new Journal(null, "su", null, null, null, null, "修改用户"));
		} catch (Exception e) {
			count = 0;
		}
		if (count > 0) {
	...
	public String upload(HttpServletRequest req,MultipartFile heardImg,User user) throws IllegalStateException, IOException{
		...
		if(filename.length()>0){
			...
			journalService.insert(new Journal(null, user.getUname(), null, null, null, null, "修改头像"));
	...
	public User alterUser(HttpServletRequest req, @RequestBody User user) throws Exception {
		...
		journalService.insert(new Journal(null, user.getUname(), null, null, null, null, "修改密码"));
		User u = service.login(user);
		return u;
	}
	...
	public String insertUser(HttpServletRequest req, User user) {
		...
		try {
			...
			journalService.insert(new Journal(null, "su", null, null, null, null, "添加用户"));
		} catch (Exception e) {
			count = 0;
		}
		if (count > 0) {
