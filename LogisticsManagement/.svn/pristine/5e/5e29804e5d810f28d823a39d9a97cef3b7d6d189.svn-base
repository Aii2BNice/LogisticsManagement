...
<div class="admin-biaogelist">
	...
	<form class="am-g" action="<%=request.getContextPath()%>/updateProduct.action?act=update" method="post">
		<table width="90%" class="am-table-bordered am-table-radius am-table-striped am-table-hover">
			<tr>
				<td>商品编号:</td>
				<td>${pro.id}<input type="text" style="display:none;" name="id" value="${pro.id}" required="required"/></td>
			</tr>
			<tr>
				<td>商品名称:</td>
				<td><input type="text" name="name" value="${pro.name}" required="required"/></td>
			</tr>
			<tr>
				<td>计量单位:</td>
				<td><input type="text" name="unit" value="${pro.unit}" required="required"/></td>
			</tr>
			<tr>
				<td>商品所属分类:</td>
			...
			<tr>
				<td>商品售价:</td>
				<td><input type="text" id="price" name="price" value="${pro.price}" required="required"/></td>
			</tr>
			<tr>
				<td>原价:</td>
				<td><input type="text" id="originalPrice" required="required" name="originalPrice" value="${pro.originalPrice}"/></td>
			</tr>
			<tr>
				<td>折扣:</td>
				<td><input type="text" id="discount" required="required" name="discount" value="${pro.discount}"/></td>
			</tr>
			<tr>
				<td>成本价:</td>
				<td><input type="text" name="cost" required="required" value="${pro.cost}"/></td>
			</tr>
			<tr>
				<td>商品型号:</td>
				<td><input type="text" name="type" required="required" value="${pro.type}"/></td>
			</tr>
			<tr>
				<td>生产厂商:</td>
				<td><input type="text" name="producerName" required="required" value="${pro.producerName}"/></td>
			</tr>
			<tr>
				<td>生产日期:</td>
				<td><input type="text" name="manufactureDate" required="required" value="<fmt:formatDate value="${pro.manufactureDate}" pattern="yyyy-MM-dd"/>" /></td>
			</tr>
			<tr>
				<td>保质期:</td>
				<td><input type="text" name="productLife" required="required" value="${pro.productLife}"/></td>
			</tr>
			<tr>
				<td>供应商:</td>
