		...
		<table width="90%" class="am-table-bordered am-table-radius am-table-striped am-table-hover"> <!-- class="am-table am-table-bordered am-table-radius am-table-striped am-table-hover" -->
			...
			<tr>
				...
				<td>
					...
					<option value="3" <c:if test="${msg==3 }">selected</c:if>>已调度</option>
					<option value="4" <c:if test="${msg==4 }">selected</c:if>>中心库房出库</option>
					<option value="5" <c:if test="${msg==5 }">selected</c:if>>分站库房到货</option>
					<option value="6" <c:if test="${msg==6 }">selected</c:if>>任务已分配</option>
					<option value="7" <c:if test="${msg==7 }">selected</c:if>>已领货</option>
					<option value="8" <c:if test="${msg==8 }">selected</c:if>>已完成</option>
					<option value="9" <c:if test="${msg==9 }">selected</c:if>>失败</option>
					</select>
				</td>
