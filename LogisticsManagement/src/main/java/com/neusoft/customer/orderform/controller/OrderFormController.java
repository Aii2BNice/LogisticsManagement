...
public class OrderFormController {
	...
	public String doInsert(HttpServletRequest req, OrderForm orderForm) {
		...
		if(orderForm.getOrderTypeId()==1){
			journalService.insert(new Journal(null, orderForm.getUname(), orderForm.getOrderId(), null, null, null, "新订订单"));
		}else if(orderForm.getOrderTypeId()==2){
			journalService.insert(new Journal(null, orderForm.getUname(), orderForm.getOrderId(), null, null, null, "退订订单"));
		}else if(orderForm.getOrderTypeId()==3){
			journalService.insert(new Journal(null, orderForm.getUname(), orderForm.getOrderId(), null, null, null, "换货订单"));
		}else{
			journalService.insert(new Journal(null, orderForm.getUname(), orderForm.getOrderId(), null, null, null, "退货订单"));
		}
		if (count > 0) {
	...
	public String update(HttpServletRequest req, OrderForm orderform) {
		...
		if (order.getOrderStateId() == 1 || order.getOrderStateId() == 2) {
			...
			journalService.insert(new Journal(null, "su", orderform.getOrderId(), null, null, null, "退订订单"));
