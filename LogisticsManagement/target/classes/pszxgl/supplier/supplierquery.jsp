...
<div class="admin-biaogelist">
	...
	<form class="am-g" action="<%=request.getContextPath()%>/suppage.action?act=query" method="post">
	...
	<table width="90%" class="am-table-bordered am-table-radius am-table-striped am-table-hover">
		...
		<c:choose>
			...
			<c:otherwise>
					<c:forEach items="${sessionScope.suplist}" var="sup">
						<tr>
							...
							<td>${sup.remark}</td>
							<td>
								<button class="am-btn am-btn-default am-btn-xs am-text-success am-round" value="${sup.id}" onclick="doUpdate(this)" >修改</button>
								<button class="am-btn am-btn-default am-btn-xs am-text-success am-round" value="${sup.id}" onclick="doDelete(this)" >删除</button>
							</td>
						</tr>
					</c:forEach>
			</c:otherwise>
			...
			<!-- 如果下面按钮有 class="ui_input_btn01"属性的话，则可以自动隐藏 -->
				...
				<input class="button-style" type="button" value="上一页" onclick="doJump(${supplier.pageNum - 1});"  />
				<input class="button-style" type="button" value="下一页" onclick="doJump(${supplier.pageNum + 1});" />
				<input class="button-style" type="button" value="尾页" onclick="doJump(${supplier.pages});" />
			<!--如果是最后一页，则只显示首页、上一页 -->
