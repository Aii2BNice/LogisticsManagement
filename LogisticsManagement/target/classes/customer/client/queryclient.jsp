	...
	<div id="main">
		...
		<div class="admin-content admin-biaogelist" id="main-right" >
			<div class="listbiaoti am-cf">
				<ul class="am-icon-flag on">查询客户</ul>
				<dl class="am-icon-home" style="float: right;">当前位置： 客户服务中心 ><a href="#">客户管理</a> ><a href="#">查询客户</a></dl>
			</div>
			<h2>客户查询</h2><br /><br />
			<form action="<%=request.getContextPath()%>/queryclient.action"
				...
				<table class="am-table-bordered am-table-radius am-table-striped am-table-hover">
					...
					<tr>
						<td colspan="3">
							<input type="submit" value="查询客户" class=".button-style button-style"/><br />
						</td>
					</tr>
					<tr>
						<td colspan="3">
					...
					<c:otherwise>
						<c:forEach items="${sessionScope.clients}" var="c">
							<tr>
								...
								<td>${c.zipCode}</td>
								<td>
									<button value="${c.clientId}" onclick="doUpdate(this)" class="am-btn am-btn-default am-btn-xs am-text-success am-round">修改</button>
									<button value="${c.clientId}" onclick="doDelete(this)" class="am-btn am-btn-default am-btn-xs am-text-success am-round">删除</button>
								</td>
							</tr>
						</c:forEach>
					</c:otherwise>
